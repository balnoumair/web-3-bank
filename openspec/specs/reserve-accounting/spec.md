# reserve-accounting Specification

## Purpose

An internal, double-entry ledger of the Treasury Service's cross-chain USDC reserves, held in `treasury.*` Postgres. It models each chain's reserve plus a shared in-transit account (value mid-bridge), so the books always balance even while a `bridgeReserve` is in flight, and it is continuously reconciled against on-chain `reserveDepth()` to surface discrepancies. The ledger is a **secondary mirror** — the chain remains the source of truth; the ledger never moves funds and never gates a bridge.
