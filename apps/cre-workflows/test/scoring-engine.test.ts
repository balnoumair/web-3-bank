...
describe("scoring engine", () => {
  it.each(SCORING_ENGINE_CASES)(
    "matches deterministic fixture: $id",
    (testCase) => {
      ...
      expect(
        result.ranked.map(({ chain, score }) => ({ chain, score })),
      ).toEqual(testCase.expected.ranked);
  ...
  it("excludes decommissioned chains from scoring entirely", () => {
    const result = scoreRunV1({
      runId: "run-decommissioned-skip-001",
      customerId: "customer-16",
      scenario: "normal",
      allowedChains: ["base-sepolia", "arbitrum-sepolia"],
      decommissionedChains: ["arbitrum-sepolia"],
      chainMetrics: [
        {
          chain: "base-sepolia",
          feeRaw: 20,
    ...
    expect(result.ranked.map((entry) => entry.chain)).toEqual(["base-sepolia"]);
    expect(result.recommendedChain).toBe("base-sepolia");
  });

  it("skips decommissioned chains when publishing activation decisions", () => {
    const decision = classifyActivationByThreshold({
      threshold: 0.6,
      decommissionedChains: ["arbitrum-sepolia"],
      ranked: [
